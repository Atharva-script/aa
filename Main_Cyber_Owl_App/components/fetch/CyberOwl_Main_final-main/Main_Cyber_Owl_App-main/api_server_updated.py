...
try:
    ...
    ABUSE_DETECTOR_AVAILABLE = True
    print("✓ Advanced Abuse Detector module loaded.")
except Exception as e:
    print(f"Warning: Advanced Abuse Detector failed to load: {e}")
    ABUSE_DETECTOR_AVAILABLE = False
...
def monitoring_worker(device_id='default'):
    """Background worker for real-time audio monitoring using AdvancedAbuseDetector.
    Captures both system audio (WASAPI loopback) and physical microphone,
    transcribes speech, and evaluates for abuse using ML pipeline + keywords.
    """
    logger.info("=" * 60)
    logger.info("MONITORING WORKER STARTED (AdvancedAbuseDetector)")
    logger.info("=" * 60)

    session_start = time.time()

    # Check if AdvancedAbuseDetector is available
    if not ABUSE_DETECTOR_AVAILABLE:
        logger.warning("AdvancedAbuseDetector not available. Falling back to test mode.")
        _run_test_mode(device_id)
        return

    try:
        # Create detector instance with ML pipeline integration
        detector = AdvancedAbuseDetector(
            lang="en-IN",
            use_loopback=True,
            use_mic=True,
            external_predict_fn=predict_toxicity if AUDIO_AVAILABLE else None,
            sample_rate=16000,
            chunk_seconds=2.5
        )

        # --- Abuse alert callback: handles DB persist, socket emit, dashboard alerts ---
        def on_audio_abuse(result):
            """Called by AdvancedAbuseDetector when abuse is detected in audio."""
            try:
                text = result.get('text', '')
                label = result.get('label', 'Bullying')
                score = result.get('score', 0.85)
                matched = result.get('matched', None)
                source_type = result.get('source', 'live')
                ...
                alert = {
                    ...
                    'score': score,
                    'latency_ms': 0,
                    'matched': matched if matched else True,
                    'sentence': text,
                    'type': 'abuse'
                }
                    ...
                    if db is not None:
                        ...
                        if user_email:
                            try:
                                ...
                                logger.error(f"Error resolving parent email: {e}")
                        else:
                        ...
                        try:
                            ...
                            if db:
                                ...
                                if user and user.get('parent_email'):
                                    socketio.emit('alert', alert, room=f"user_{user.get('parent_email')}")
                        except Exception:
                            pass
                    else:
                ...
                logger.error(f"Error in on_audio_abuse callback: {e}")

        # --- Transcript callback: logs all speech (abusive or clean) ---
        def on_transcript(result):
            """Called for every transcribed audio chunk (abusive or not)."""
            try:
                text = result.get('text', '')
                if not text:
                    return
        ...
        # Register callbacks
        detector.on_abuse_detected(on_audio_abuse)
        detector.on_transcript(on_transcript)

        # Start the detector (spawns loopback + mic threads internally)
        started = detector.start_realtime_audio_monitoring()

        if not started:
            logger.warning("AdvancedAbuseDetector failed to start any audio source. Falling back to test mode.")
            _run_test_mode(device_id)
            return

        # Keep this thread alive while monitoring is active
        while get_device_state(device_id)['running']:
            time.sleep(1)

        # Stop detector when monitoring stops
        detector.stop_realtime_audio_monitoring()

    except Exception as e:
        logger.error(f"AdvancedAbuseDetector error: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Flask socketio.run() returned (this should never print)", flush=True)
    except Exception as e:
