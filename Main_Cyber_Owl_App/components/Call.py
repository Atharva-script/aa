...
# Global lock for detector initialization
...
logger_nudity.propagate = False # [NEW] Stop logs from reaching the terminal via propagation
...
FEMALE_UNSAFE = [
    ...
    'belly', 'exposed_belly', 'armpits', 'exposed_armpits', 'umbilicus', 'exposed_umbilicus', 'thigh', 'exposed_thigh'
]
...
def init_detector():
    ...
    with detector_lock:
        if detector is None:
            ...
            try:
                # Check for model existence to warn about download
                home = Path.home()
                model_dir = home / ".nudenet"
                if not model_dir.exists():
                    print(f"[NUDITY] 📥 NudeNet model not found at {model_dir}. Downloading (this may take several minutes)...", flush=True)
                    log(f"NudeNet model not found. Downloading...")
    ...
    for label, score, area_frac, raw in parsed_detections:
        ...
        if score >= CONF_THRESHOLD:
            ...
            if matched:
                ...
                log(f"✓ UNSAFE DETECTED: '{label}' matched keyword '{matched_kw}' (score={score:.2f})", "info")
            else:
                log(f"✗ No match for '{label}' (score={score:.2f}) - not in unsafe list", "info")
        else:
            log(f"DEBUG: Score too low: {label}, score={score:.3f} < {CONF_THRESHOLD}", "info")
                
    is_safe = (len(reasons) == 0)
    log(f"DEBUG: Final result: is_safe={is_safe}, reasons={len(reasons)}", "info")
...
def detection_worker():
    ...
    logger_nudity.info("Worker thread started")
    try:
        init_detector() # Init once
        logger_nudity.info("NudeDetector initialized and ready")
    except Exception as e:
        log(f"Detector init failed: {e}. Worker exiting.", "error")
        logger_nudity.error(f"ERROR: {e}")
        return
        ...
        try:
            ...
            if not is_safe:
                ...
                if (now - last_alert_time) > COOLDOWN_SECONDS:
                    ...
                    logger_nudity.warning(f"⚠️ ALERT: {reason_str}")
...
def monitor_screen_forever(stop_flag=None, on_alert=None):
    ...
    logger_nudity.info(f"Screen monitoring started (20 FPS, {DETECT_RESIZE[0]}x{DETECT_RESIZE[1]} resize, threshold={CONF_THRESHOLD}, cooldown=5s)")
        ...
        while True:
            ...
            try:
                if stop_flag and hasattr(stop_flag, 'is_set') and stop_flag.is_set():
                    ...
                    # Send sentinel to shutdown worker
                    frame_queue.put(None)
                    break
            except Exception as e:
                log(f"Error checking stop_flag: {e}", "warning")
