...
class AbuseDetectionService {
  ...
  Future<Map<String, dynamic>> startMonitoring({String? userEmail}) async {
    ...
    try {
      ...
      final resolvedEmail = userEmail ?? localUser?['email'] ?? 'pc_user';
      ...
      final response = await http
          .post(
            ...
            body: json.encode({
              'email': resolvedEmail,
              'user_email': resolvedEmail, // [SYNC] Backend looks for this
            }),
          )
  ...
  void _setupSocket(String? deviceId) {
    ...
    _socket!.onConnect((_) async {
      ...
      // --- FIX: Join BOTH device room AND user room ---
      // The backend emits alerts to 'user_{email}' rooms,
      // so we MUST send the email to join the correct room.
      final joinData = <String, dynamic>{};
      if (deviceId != null) {
        joinData['device_id'] = deviceId;
      }

      // Resolve the user's email from AuthService
      try {
        final userData = await AuthService.getLocalUser();
        if (userData != null) {
          final userEmail = userData['email']?.toString();
          if (userEmail != null && userEmail.isNotEmpty) {
            joinData['email'] = userEmail;
            if (kDebugMode) {
              print('Socket joining user room: user_$userEmail');
            }
          }

          // Also try to get the parent email to receive child alerts
          final parentEmail = userData['parent_email']?.toString();
          if (parentEmail != null && parentEmail.isNotEmpty) {
            // Join parent room too so parent sees child alerts
            // Send a second join for the parent email
            _socket!.emit('join', {'email': parentEmail});
            if (kDebugMode) {
              print('Socket also joining parent room: user_$parentEmail');
            }
          }
        }
      } catch (e) {
        if (kDebugMode) {
          print('Socket: Could not resolve user email: $e');
        }
      }
    ...
    // --- FIX: Listen for real-time 'alert' events from backend ---
    _socket!.on('alert', (data) {
      if (kDebugMode) {
        print('Socket alert received: $data');
      }
      if (data != null && data is Map<String, dynamic>) {
        try {
          // Validate it's a proper alert by parsing (don't need the result)
          AbuseAlert.fromJson(data);
          // Push to alerts stream so UI updates immediately
          if (!_alertsController.isClosed) {
            // Fetch full list to stay consistent with polling
            getAlerts(deviceId: deviceId).then((alerts) {
              if (!_alertsController.isClosed) _alertsController.add(alerts);
            });
          }
          ...
          if (kDebugMode) {
            print('Socket alert parse error: $e');
          }
        }
      }
    });
