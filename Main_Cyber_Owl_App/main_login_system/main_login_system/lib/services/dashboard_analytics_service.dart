...
class DashboardAnalyticsService {
  ...
  Future<DashboardAnalytics> fetchAnalytics({String? deviceId, String? childEmail}) async {
    try {
      String url = '$baseUrl/analytics/overview?';
      if (deviceId != null) url += 'device_id=$deviceId&';
      if (childEmail != null) url += 'child_email=$childEmail&';
  ...
  /// Fetch timeline data
  Future<TimelineData> fetchTimeline({String? deviceId, String? childEmail}) async {
    try {
  ...
  /// Fetch heatmap data
  Future<HeatmapData> fetchHeatmap({String? deviceId, String? childEmail}) async {
    try {
  ...
  void startPolling(
      {Duration interval = const Duration(seconds: 3), String? deviceId, String? childEmail}) {
    stopPolling();
    
    // Initial fetch
    fetchAnalytics(deviceId: deviceId, childEmail: childEmail);
    fetchTimeline(deviceId: deviceId, childEmail: childEmail);
    fetchHeatmap(deviceId: deviceId, childEmail: childEmail);
